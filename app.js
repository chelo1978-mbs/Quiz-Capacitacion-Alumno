...
function initDashboardPage(tbody) {
  ...
  // Escuchar botón de realizar nuevamente
  const btnRealizarNuevamente = document.getElementById("btn-realizar-nuevamente-dash");
  if (btnRealizarNuevamente) {
    btnRealizarNuevamente.addEventListener("click", () => {
      localStorage.removeItem("participante_id");
      localStorage.removeItem("participante_actual");
      localStorage.removeItem("quiz_secuencia_index");
      localStorage.removeItem("respuestas_acumuladas");
...
// Renderizar filas de la tabla del Dashboard de Resultados
function renderizarTablaDashboard(tbody, lista) {
  ...
  tbody.innerHTML = lista.map((res) => {
    ...
    const correctasText = res.correctas !== undefined ? `${res.correctas} aciertos` : "-";
    ...
    return `
      <tr>
        ...
        <td style="font-weight: 600; color: var(--text-main);">${escapeHtml(correctasText)}</td>
        ...
        <td style="text-align: center;">
          <button class="btn btn-outline btn-ver-detalle" data-id="${res.id}" style="padding: 0.4rem 0.95rem; font-size: 0.85rem; font-weight: 700; border-radius: 6px;">
